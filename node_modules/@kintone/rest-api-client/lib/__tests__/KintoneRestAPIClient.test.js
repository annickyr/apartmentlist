    ...
    describe("constructor", function () {
        describe("Header", function () {
            ...
            it("ApiToken auth", function () {
                ...
                var client = new KintoneRestAPIClient_1.KintoneRestAPIClient({ baseUrl: baseUrl, auth: auth });
            ...
            it("Password  auth", function () {
                ...
                var client = new KintoneRestAPIClient_1.KintoneRestAPIClient({ baseUrl: baseUrl, auth: auth });
                expect(client.getHeaders()).toEqual({
                    "X-Cybozu-Authorization": js_base64_1.Base64.encode(USERNAME + ":" + PASSWORD)
                });
            });
            ...
            it("Basic auth", function () {
                ...
                var client = new KintoneRestAPIClient_1.KintoneRestAPIClient({ baseUrl: baseUrl, basicAuth: basicAuth });
                expect(client.getHeaders()).toEqual({
                    Authorization: "Basic " + js_base64_1.Base64.encode("user:password"),
                    "X-Requested-With": "XMLHttpRequest"
                });
            });
            it("should use Session auth if auth param is not specified", function () {
                var client = new KintoneRestAPIClient_1.KintoneRestAPIClient({ baseUrl: baseUrl });
                expect(client.getHeaders()).toEqual({
                    "X-Requested-With": "XMLHttpRequest"
                });
            });
            ...
            it("should raise an error in Node environment if baseUrl param is not specified", function () {
                ...
                expect(function () { return new KintoneRestAPIClient_1.KintoneRestAPIClient(); }).toThrow("in Node environment, baseUrl is required");
            });
        });
    });
});
